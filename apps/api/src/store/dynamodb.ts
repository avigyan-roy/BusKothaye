...
export class DynamoJourneyRepository implements JourneyRepository {
  ...
  async createAccount(account: AccountRecord): Promise<boolean> {
    try {
      ...
      throw new StorageUnavailableError(
        `Could not create account: ${errorName(error)}`,
      );
    }
  }
  ...
  async getAccountByUsername(
    ...
    try {
      ...
      throw new StorageUnavailableError(
        `Could not read account: ${errorName(error)}`,
      );
    }
  }
  ...
  async putAccount(
    ...
    try {
      ...
      if (isConditionalFailure(error)) return false;
      throw new StorageUnavailableError(
        `Could not update account: ${errorName(error)}`,
      );
    }
  }
  ...
  async createAccountSession(session: AccountSessionRecord): Promise<void> {
    try {
      ...
      throw new StorageUnavailableError(
        `Could not create session: ${errorName(error)}`,
      );
    }
  }
  ...
  async getAccountSession(
    ...
    try {
      ...
      const session = result.Item?.["session"] as
      ...
      throw new StorageUnavailableError(
        `Could not read session: ${errorName(error)}`,
      );
    }
  }
  ...
  async deleteAccountSession(tokenHash: string): Promise<void> {
    try {
      ...
      throw new StorageUnavailableError(
        `Could not delete session: ${errorName(error)}`,
      );
    }
  }

  async getDemoControl(
    defaultValue: DemoControlRecord,
  ): Promise<DemoControlRecord> {
    ...
    try {
      ...
      if (existing.Item?.["control"])
        return existing.Item["control"] as DemoControlRecord;
      try {
        await this.client.send(
          new PutCommand({
            TableName: this.table,
        ...
        return (
          (raced.Item?.["control"] as DemoControlRecord | undefined) ??
          defaultValue
        );
      }
    } catch (error) {
      throw new StorageUnavailableError(
        `Could not read demo control: ${errorName(error)}`,
      );
    }
  }

  async putDemoControl(
    next: DemoControlRecord,
    ...
    try {
      ...
      if (isConditionalFailure(error)) return false;
      throw new StorageUnavailableError(
        `Could not write demo control: ${errorName(error)}`,
      );
    }
  }
    ...
    try {
      await this.client.send(
        new UpdateCommand({
          ...
          UpdateExpression: "SET control.lease = :lease",
          ConditionExpression:
      ...
      if (isConditionalFailure(error)) return false;
      throw new StorageUnavailableError(
        `Could not acquire demo lease: ${errorName(error)}`,
      );
    }
  }
  ...
  async createJourney(
    ...
    { created: true } | { created: false; existing: IdempotencyRecord }
  > {
    if (idempotency) {
    ...
    > = [
      {
        Put: {
          ...
          Item: {
            ...
            ttl: toEpochSeconds(
              snapshot.createdAtMs + JOURNEY_TOMBSTONE_MS * 2,
            ),
          },
          ConditionExpression: "attribute_not_exists(PK)",
        },
  ...
  async getJourney(journeyId: string): Promise<JourneySnapshot | null> {
    try {
      ...
      throw new StorageUnavailableError(
        `Could not read journey: ${errorName(error)}`,
      );
    }
  }
  ...
  async putJourney(
    ...
    try {
      ...
      this.lastWriteOk = false;
      throw new StorageUnavailableError(
        `Could not write journey: ${errorName(error)}`,
      );
    }
  }
  ...
  async recordJoin(
    ...
    { written: true } | { written: false; existing: IdempotencyRecord | null }
  > {
    if (!idempotency) {
    ...
    try {
      ...
      this.lastWriteOk = false;
      throw new StorageUnavailableError(
        `Could not record join: ${errorName(error)}`,
      );
    }
  }
    ...
    do {
      ...
      records.push(
        ...(result.Items ?? [])
          ...
          // TTL deletion is asynchronous, so expired records are filtered here
          // rather than assumed gone.
          .filter(
            (record) => nowMs - record.serverTs <= RAW_REPORT_RETENTION_MS,
          ),
      );
  ...
  private async getIdempotency(
    ...
    if (this.nowMs() - record.createdAtMs > IDEMPOTENCY_RETENTION_MS)
      return null;
    return record;
  }
  ...
  private async listMembership(pk: string): Promise<JourneySnapshot[]> {
    try {
      ...
      do {
        ...
        ids.push(
          ...(result.Items ?? []).map((item) => String(item["journeyId"])),
        );
        exclusiveStartKey = result.LastEvaluatedKey as
          | Record<string, unknown>
          | undefined;
      } while (exclusiveStartKey);
      return await this.batchGetJourneys(ids);
    } catch (error) {
      throw new StorageUnavailableError(
        `Could not list journeys: ${errorName(error)}`,
      );
    }
  }

  private async batchGetJourneys(
    journeyIds: readonly string[],
  ): Promise<JourneySnapshot[]> {
    const snapshots: JourneySnapshot[] = [];
    for (let start = 0; start < journeyIds.length; start += BATCH_GET_LIMIT) {
      let pendingKeys = journeyIds
      ...
      for (let attempt = 0; pendingKeys.length > 0; attempt += 1) {
        ...
        snapshots.push(
          ...(result.Responses?.[this.table] ?? [])
            .map((item) => item["snapshot"] as JourneySnapshot | undefined)
            .filter(
              (snapshot): snapshot is JourneySnapshot => snapshot !== undefined,
            )
            .filter(
              (snapshot) =>
                snapshot.endedAtMs === null ||
        ...
        pendingKeys = (result.UnprocessedKeys?.[this.table]?.Keys ??
          []) as typeof pendingKeys;
        if (pendingKeys.length === 0) break;
        if (attempt >= BATCH_GET_MAX_RETRIES) {
          throw new StorageUnavailableError(
            "DynamoDB returned unprocessed journey reads",
          );
        }
