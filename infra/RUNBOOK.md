...
- Amazon Location Maps V2 and Routes V2 with an origin/action-restricted API key.
...
## 4. Configure Amazon Location Service

In the AWS account and `ap-south-1` region:

1. Create a production Location API key named `buskothay-web`.
2. Allow only Maps V2 style/tile actions and the Routes V2 calculation action used by the app.
3. Restrict allowed origins to the exact Amplify preview/main origins. Put localhost only on a separate development key.
4. Set service quotas plus AWS budget alerts that fit the checked cost model.
5. Save the key and region in the Amplify build environment, not in Git.

This is a public browser API key—not an AWS access key. It is expected to appear in the built JavaScript, so origin/action restrictions and quota monitoring are mandatory. Review current AWS terms before retaining route geometry generated by the admin console.
...
VITE_LOCATION_API_KEY=YOUR_RESTRICTED_LOCATION_API_KEY
```

6. Deploy. The guarded build fails if the API still points at localhost, the
   provider is not Amazon, the Location key is missing, or the region is invalid.
...
9. Add the exact Amplify origin to the Location key's allowed origins.
...
- Amazon tiles, dark/light schemes, traffic, markers, and attribution load; an unapproved origin is rejected.
