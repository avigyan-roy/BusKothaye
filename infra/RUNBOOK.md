...
- Google Maps Platform with a referrer/API-restricted browser key and map ID.
...
## 4. Configure Google Maps Platform

In a Google Cloud project with billing enabled:

1. Enable the Maps JavaScript API and the Routes functionality required by the JavaScript Routes library.
2. Create a production map ID for Advanced Markers.
3. Create a browser key named `buskothay-web` and restrict it to those APIs.
4. Restrict HTTP referrers to the exact Amplify preview/main origins. Put localhost only on a separate development key.
5. Set Maps and Routes quotas plus Google Cloud billing alerts that fit the checked cost model.
6. Save the key and map ID in the Amplify build environment, not in Git.

This is a public browser key—not an AWS access key. It is expected to appear in the built JavaScript, so referrer/API restrictions and quota monitoring are mandatory. Review current Google terms before retaining route geometry generated by the admin console.
...
6. Deploy. The guarded build fails if the API still points at localhost, the
   Google key is missing, or the map ID is still `DEMO_MAP_ID`.
...
9. Add the exact Amplify origin pattern to the Google key's allowed HTTP referrers.
...
- Google tiles, dark scheme, traffic, markers, and attribution load; an unapproved referrer is rejected.
- The administrator can open `/admin/routes`, drag a stop, generate an ordered road path, and publish a validated new revision without exposing the browser key in logs.
